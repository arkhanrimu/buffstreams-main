...
fetch(apiURL)
  ...
  .then(matches => {
    ...
    matches.forEach(match => {
      ...
      else if (diffMinutes >= 0 && diffMinutes < 150) { status = "live"; liveCount++; }
      ...
      allMatches.push({
        ...
        match: match.title || "-",
        status,
        url: "https://arkhanrimu.github.io/buffstreams-watch-page/?id=" + match.id,
    ...
    function renderMatches(filter) {
      ...
      if (filtered.length === 0) {
        ...
        filtered.forEach(m => {
          const badge = m.status === "finished" ? '<span class="badge finished"></span>' : m.status === "live" ? '<span class="badge live"></span>' : "";
          const row = document.createElement("tr");
          row.innerHTML = '<td>' + m.time + '</td><td>' + m.sport + '</td><td>' + m.tournament + '</td><td>' + m.match + ' ' + badge + '</td><td><a class="watch-btn" target="_blank" href="' + m.url + '">Watch</a></td>';
